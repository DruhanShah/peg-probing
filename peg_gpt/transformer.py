...
class Transformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.embed_enc = nn.Embedding(cfg.s_vocab, cfg.d_m)
        self.embed_dec = nn.Embedding(cfg.t_vocab, cfg.d_m)
        self.positional_encoding = PositionalEncoding(cfg.d_m, cfg.N_max)

        self.encoders = nn.ModuleList([
            Encoder(cfg.d_m, cfg.d_mlp, cfg.n_h, cfg.dropout)
            for _ in range(cfg.L_enc)
        ])
        self.decoders = nn.ModuleList([
            Decoder(cfg.d_m, cfg.d_mlp, cfg.n_h, cfg.dropout)
            for _ in range(cfg.L_dec)
        ])

        self.fc = nn.Linear(cfg.d_m, cfg.t_vocab)
        self.dropout = nn.Dropout(cfg.dropout)

    def mask_gen(self, src, tgt):
        mask_s = (src != 0).unsqueeze(1).unsqueeze(2)
        mask_t = (tgt != 0).unsqueeze(1).unsqueeze(3)
        n = tgt.size(1)
        nopeek_mask = (1 - torch.triu(torch.ones(1, n, n), diagonal=1)).bool()
        mask_t = mask_t & nopeek_mask
    ...
    def generate(self, prompt, max_len):
        ...
        for _ in range(max_len):
            mask_s, mask_t = self.mask_gen(prompt, prompt)
            s_embed = self.dropout(self.positional_encoding(self.embed_enc(prompt)))
            t_embed = self.dropout(self.positional_encoding(self.embed_dec(prompt)))
            enc_output = s_embed
            for encoder in self.encoders:
                enc_output = encoder(enc_output, mask_s)
            dec_output = t_embed
