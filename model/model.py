...
class Model(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.token_embeds = nn.Embedding(cfg.d_vocab, cfg.d_m, dtype=cfg.dtype)
        self.pos_embeds = nn.Embedding(cfg.n_ctx, cfg.d_m, dtype=cfg.dtype)
        ...
        self.head = nn.Linear(cfg.d_m, cfg.d_vocab, dtype=cfg.dtype)
    ...
    def _register_hooks(self):
        ...
        add_hook(self.token_embeds, "token_embeds")
        add_hook(self.pos_embeds, "pos_embeds")
    ...
    def forward(self, x, y=None, return_type=["logits"]):
        ...
        token_embeds = self.token_embeds(x.to(torch.long))
        position_ids = torch.arange(0, N,
                                    dtype=torch.long, device=self.cfg.device)
        pos_embeds = self.pos_embeds(position_ids).to(self.cfg.dtype)
        ...
        if "loss" in return_type:
            returnable["loss"] = (self.loss(logits, y)
                                  if y is not None
                                  else None)
